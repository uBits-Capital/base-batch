...
import { USDC_CONTRACT, ARBISCAN_URL } from "@/config";
...
export default function RemoveLiquidity({
  ...
  } = useQuote(positionData.fee, new Percent(1, 100));
  ...
  useEffect(() => {
    ...
    if (token0 !== USDC_CONTRACT && removeAmount0 > JSBI.BigInt(0)) {
      quoteMaxAmountOut0(
        token0,
        USDC_CONTRACT,
        removeAmount0.toString(),
      ).then();
    }
    if (token1 !== USDC_CONTRACT && removeAmount1 > JSBI.BigInt(0)) {
      quoteMaxAmountOut1(
        token1,
        USDC_CONTRACT,
        removeAmount1.toString(),
      ).then();
    }
  }, [
    positionData,
  ...
  useEffect(() => {
    setDisabled(!(!loadingRemoveLiquidity && !quote0Loading && !quote1Loading));
  }, [loadingRemoveLiquidity, quote0Loading, quote1Loading]);
  ...
  return (
    ...
    >
      <div
        ...
        aria-hidden="true"
      ></div>
      {!loadingRemoveLiquidity && !showTxModal && (
                ...
                <div className="flex flex-col justify-start items-start w-[320px] md:w-[445px] overflow-hidden rounded-2xl bg-[#12131a]">
                  <div className="flex justify-between items-center self-stretch flex-grow-0 flex-shrink-0 relative p-6 border-t-0 border-r-0 border-b border-l-0 border-[#273345]">
                    ...
                    <p className="flex-grow w-[333px] text-xl font-medium text-center text-white">
                      {!closingPosition ? "Remove liquidity" : "Closing Position"}
                    </p>
                    <div
                  ...
                  <div className="flex flex-col justify-center items-center self-stretch flex-grow-0 flex-shrink-0 gap-5 p-6">
                    {closingPosition && (
                      <div className="flex flex-col w-full  justify-center items-center gap-3 p-2">
                        ...
                        <p className="flex-grow-0 flex-shrink-0  text-sm text-center text-[#ff9500]">
                    ...
                    {!closingPosition && (error && slippageCheckError ? (
                      <button
                        className={`flex justify-center items-center self-stretch flex-grow-0 flex-shrink-0 gap-1 px-4 py-3 rounded-[1000px]  ${!disabled ? "text-[#12131A] bg-white" : "text-gray-500 bg-[#273345]"}`}
                        disabled={disabled}
                        onClick={() => handleRemoveLiquidity(true)}
                      >
                      ...
                      <button
                        ...
                        onClick={() => handleRemoveLiquidity(false)}
                      >
                        <p className="flex-grow-0 flex-shrink-0 text-base font-semibold text-left   ">
                          Remove
                        </p>
                    ...
                    {closingPosition && <button className={`flex justify-center items-center self-stretch flex-grow-0 flex-shrink-0 gap-1 px-4 py-3 rounded-[1000px]  ${!disabled ? "text-white bg-[#a31111]" : "text-gray-500 bg-[#273345]"}`}
                      disabled={disabled}
                      onClick={() => handleRemoveLiquidity(true)}
                    >
                      <p className="flex-grow-0 flex-shrink-0 text-base font-semibold text-left ">
      ...
      {showTxModal && !slippageCheckError && (
        <TransactionModal
          ...
          modalType={ModalActionType.REMOVE_LIQUIDITY}
          close={(type) => {
            setShowTxModal(false);
          ...
          token0Ticker={isWETH(positionData.token0.id) ? "ETH" : positionData.token0.symbol}
          token1Ticker={isWETH(positionData.token1.id) ? "ETH" : positionData.token1.symbol}
          token0Amount={fromInt(
            removeAmount0,
            positionData.token0.decimals,
          ).toString()}
          token1Amount={fromInt(
            removeAmount1,
            positionData.token1.decimals,
          ).toString()}
          usdcAmount={showUsdc ? usdcAmount : undefined}
